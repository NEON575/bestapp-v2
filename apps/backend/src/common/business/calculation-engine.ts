...
export function normalizeCalculationValues(input: Partial<CalculationFormValues> & { templateKey?: CalculationTemplateKey }): CalculationFormValues {
  const defaults = defaultCalculationValues(input.templateKey ?? 'certificate_sheet');
  const catalog = {
  ...
  return {
    templateKey: input.templateKey ?? defaults.templateKey,
    ...
    extraCosts: (input.extraCosts ?? defaults.extraCosts).map((item) => createExtraCostItem(item)),
    catalog: {
      ...catalog,
