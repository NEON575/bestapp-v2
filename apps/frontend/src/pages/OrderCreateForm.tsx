...
import type { CreateCustomerDto, CreateOrderDto, CustomerListItem, OrderDetail, UserSummary } from '@bestapp/shared';
...
type OrderCreateFormProps = {
  onCancel: () => void;
  onCreated: (order: OrderDetail) => void | Promise<void>;
  submitLabel: string;
  cancelLabel: string;
};

export function OrderCreateForm({ onCancel, onCreated, submitLabel, cancelLabel }: OrderCreateFormProps) {
  ...
  const validate = () => {
    ...
    if (!form.deadlineAt) nextErrors.deadlineAt = 'Təhvil tarixini seçin';
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!validate() || !item) {
      toast.warning('Forma tamam deyil', 'Vacib sahələri doldurun.');
      return;
    }
